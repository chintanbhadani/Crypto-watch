...
export default function ChartCointainer() {
  ...
  useEffect(() => {
    const url = `https://api.coingecko.com/api/v3/coins/${crypto}/market_chart?vs_currency=${currency}&days=${days}`;
    axios
      .get(url)
      .then((res) => {
  ...
  return (
    <>
      ...
      <div className=" bg-white dark:bg-stone-800 border-2 border-yellow-400 shadow-lg hover:duration-300 hover:shadow-2xl rounded-xl dark:border-transparent sm:px-10 p-0 sm:pb-10 h-full sm:min-h-fit">
        {chartType === "barHorizontal" && <Bar data={userData} height={225} options={options1} />}
        {chartType === "barVertical" && <Bar data={userData} height={225} options={options} />}
        {chartType === "line" && <Line data={userData} height={225} options={options} />}
      </div>
    </>
  );
}

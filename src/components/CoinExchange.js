...
function CoinExchange() {
  ...
  const handleSell = () => {
    const checkCoinInState = cryptoInPortfolio.find((crypto) => crypto.name === coin);
  ...
  return (
    <div className="mt-4  mb-14 dark:bg-stone-800 border-yellow-400 border-2 dark:border-0 dark:shadow-black hover:duration-300 hover:shadow-2xl bg-white shadow-lg font-[Poppins] rounded-xl p-5 w-full">
      <div className="flex flex-col md:flex-row items-center justify-between">
        <div>
          <h3 className=" font-bold dark:text-yellow-400 mt-2 text-xl ">Exchange Coins</h3>
        </div>
      </div>
      <div className="flex flex-col ">
        ...
        <div className="flex items-center justify-around space-x-5 pt-4 mt-5 ">
          ...
          <div className="relative">
            <select
              ...
              className=" bg-slate-50 rounded border appearance-none py-2 dark:bg-stone-700 dark:text-white dark:border-none text-gray-600 focus:outline-none focus:border-yellow-400 font-medium text-base pl-3 md:pl-6  pr-8 md:pr-14"
            >
        ...
        {error && <p className="flex justify-center text-red-600 my-8">{error}</p>}
        <div className="flex justify-center pt-2">
          <button
            disabled={exchangeType === "buy" ? !(buyValue > 0) : !(sellValue > 0)}
            onClick={exchangeType === "buy" ? handleBuy : handleSell}
            className="bg-yellow-400 text-stone-900 py-3 dark:bg-yellow-400 font-semibold dark:text-black dark:hover:border-yellow-400 dark:hover:text-white dark:border-stone-900 dark:hover:bg-stone-900 px-10 rounded-md hover:bg-stone-900 hover:border-2 hover:border-yellow-4.0 hover:duration-300 hover:text-white border-2 border-transparent disabled:opacity-100 mt-8"
          >

...
function PortfolioContainer() {
  ...
  const data = {
    labels: cryptoInPortfolio.map((crypto) => crypto.name.charAt(0).toUpperCase() + crypto.name.slice(1)),
    datasets: [
      {
  ...
  return (
    <div className="dark:bg-stone-800 mb-14 pl-7 border-2 border-yellow-300 dark:border-yellow-400 dark:border-0 dark:shadow-black  container shadow-lg hover:duration-300 hover:shadow-2xl bg-white mt-4 font-['Poppins'] rounded-xl p-5 w-full ">
      <div className="flex flex-col md:flex-row items-center md:m-2 justify-between flex-1">
        <div>
          <h3 className="dark:text-yellow-400 font-poppins font-bold text-xl ">Portfolio</h3>
        </div>
        <div className="flex flex-col text-xl items-center md:flex-row ">
          <p className=" text-slate-400 dark:text-yellow-400 mr-1">Total Value </p>
